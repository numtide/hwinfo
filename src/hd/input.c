...
void hd_scan_input(hd_data_t *hd_data) {
  if (!hd_probe_feature(hd_data, pr_input))
    return;
...
void get_input_devices(hd_data_t *hd_data) {
  ...
  char *name = NULL, *handlers = NULL, *key = NULL, *rel = NULL, *abso = NULL, *sysfs;
  size_t len;
  ...
  for (ok = 0, sl = input; sl; sl = sl->next) {
    ...
    if (!strncmp(sl->str, INP_SYSFS, sizeof INP_SYSFS - 1)) {
      s = sl->str + sizeof INP_SYSFS - 1;
      sysfs = canon_str(s, strlen(s));
      sysfs = all_bits(sysfs);
      continue;
    }
  }
